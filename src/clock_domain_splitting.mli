...
module Copied_circuit : sig
  (** In the new circuit, most old signals map to a single new signal. There is one
      ...
      - If an old signal is the output of a clock domain, it will have 2 new signals:
        one a copied version of the old signal and one that is a wire driven by that
        signal. *)
...
*)
val group_by_clock_domain : Circuit.t -> Copied_circuit.t Clock_domain.Map.t
...
val classify_original_uids
  :  original_circuit:Circuit.t
  -> clock_domains:Copied_circuit.t Clock_domain.Map.t
  -> Original_signal_kind.t Map.M(Signal.Uid).t

...
let%expect_test "adder - vcd" =
  ...
  let vcd = Sim.Vcd.create Stdio.stdout (I.to_list input @ O.to_list output) in
  let sim =
    create
      (processes
       @ Sim.Vcd.processes vcd
  ...
  in
  Sim.Vcd.attach_to_simulator vcd sim;
  run ~time_limit:150 sim;
  [%expect
    {|
    $date
